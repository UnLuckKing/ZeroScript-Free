...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "foreach($name in $obsolete){$p=Join-Path $root $name; if(Test-Path $p){Remove-Item $p -Force -Recurse}}; Get-ChildItem $root -File -Filter 'RELEASE*.md' -ErrorAction SilentlyContinue | Remove-Item -Force;" ^
