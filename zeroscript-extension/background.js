...
function phasesForGoal(goal) {
  ...
  const wantsMap = full || /\b(map|world|terrain|lobby|environment|lighting|spawn|zone|island|harita|altar|portal|leaderboard|presentation|onboarding)\b/.test(g);
  const wantsUi = full || /\b(ui|gui|hud|menu|panel|button|responsive|mobile|interface|arayüz|shop|gamepass|upgrade|index|presentation|onboarding|monetization)\b/.test(g);
...
function phasePrompt(task) {
  ...
  if (task.phase === "map") return `${shared}\nAct as the Map Designer for a popular Roblox simulator/RNG game. Inspect the existing world before editing. Build or improve a compact premium lobby, not a random large terrain: spawn must face the main action, the central action altar/core must be obvious, and shop, upgrades, index/collection, leaderboard, boost/gamepass, and portal/world zones must be readable from spawn through paths, lighting, signs, color, and VFX. Prefer polished Creator Store/Toolbox assets when they reduce fragile custom work. Keep collision, scale, performance, and traversal clean. Preserve correct existing gameplay objects. Playtest spawn orientation, walking routes, and Output after changes.`;
  if (task.phase === "ui") return `${shared}\nAct as the UI Designer for a premium Roblox simulator/RNG game. Inspect every relevant ScreenGui and capture the running UI when possible. Implement a clean mobile-safe interface: top currency/status bar, bottom action bar, left menu, main roll/result card, reward feed, settings, shop/upgrades/index panels, consistent icons, gradients, strokes, shadows, hover/click feedback, and clear disabled/loading states. Avoid full-screen clutter; use scale-based responsive sizing and safe areas. Preserve existing logic/remotes and test all changed buttons in play mode.`;
