...
- Verified lesson extraction after terminal tasks. Successful lessons require a completed task, sufficient evidence score and clean recorded Output.
...
- Recipe scoring that increases after verified success and decreases after failure.
...
- A task is not promoted as a successful lesson when it fails, is cancelled, lacks enough evidence or retains Output errors.
...
The repository includes Python compile checks, SQLite Memory Vault tests, control API tests, JavaScript syntax checks and a learning-snapshot test. Real Chrome provider pages, Roblox Studio visual behavior, long-term learning quality and production DataStores still require live use on the user's Windows machine.
