...
def install(hub: Any) -> None:
    ...
    def build_automation(self: Any) -> None:
        ...
        for index, (label, command) in enumerate(actions):
            ttk.Button(action_grid, text=label, command=command).grid(row=index // 3, column=index % 3, sticky="ew", padx=4, pady=4)
        ...
        ttk.Combobox(settings, textvariable=self.automation_timeout_var, values=["10", "15", "20", "30", "45", "60", "90"], state="readonly", width=8).grid(row=0, column=1, sticky="w", padx=(6, 18))
        ...
        ttk.Combobox(settings, textvariable=self.automation_recovery_var, values=["0", "1", "2", "3", "4"], state="readonly", width=6).grid(row=0, column=4, sticky="w", padx=6)
        ttk.Button(settings, text="Ayarları uygula", style="Primary.TButton", command=lambda: save_automation(self)).grid(row=0, column=5, sticky="e", padx=(18, 0))
        ...
        for index, (label, variable) in enumerate(checks):
            ttk.Checkbutton(settings, text=label, variable=variable).grid(row=1 + index // 3, column=(index % 3) * 2, columnspan=2, sticky="w", pady=4)
        settings.columnconfigure(5, weight=1)

        project_row = ttk.Frame(outer, style="Card.TFrame")
        project_row.pack(fill="x", pady=(4, 8))
        self.automation_project_label = ttk.Label(project_row, text="Açık oyun algılanıyor…", style="Card.TLabel")
        self.automation_project_label.pack(side="left")
        ttk.Button(project_row, text="Bu oyuna profil kaydet", command=lambda: save_project_profile(self)).pack(side="right")
        ttk.Button(project_row, text="Oyun profilini şimdi yükle", command=lambda: load_project_profile(self, notify=True)).pack(side="right", padx=6)
        ...
        self.automation_models = ttk.Treeview(left, columns=("success", "attempts", "avg", "errors"), show="headings", height=8)
        for key, title, width in (("success", "Başarı", 70), ("attempts", "İş", 55), ("avg", "Ort. süre", 85), ("errors", "Hata", 55)):
        ...
        self.automation_errors = hub.tk.Text(left, height=11, wrap="word", bg="#171b21", fg="#cfd5df", relief="flat", font=("Consolas", 9), padx=8, pady=8)
        self.automation_errors.pack(fill="both", expand=True, pady=(6, 0))

        ttk.Label(right, text="Tanılama sonucu", style="Status.TLabel").pack(anchor="w")
        self.automation_diagnosis = hub.tk.Text(right, height=9, wrap="word", bg="#171b21", fg="#cfd5df", relief="flat", font=("Consolas", 9), padx=8, pady=8)
        self.automation_diagnosis.pack(fill="x", pady=(6, 10))

        ttk.Label(right, text="Bildirim merkezi", style="Status.TLabel").pack(anchor="w")
        self.automation_notifications = ttk.Treeview(right, columns=("time", "level", "title"), show="headings", height=11)
        for key, title, width in (("time", "Saat", 60), ("level", "Tür", 65), ("title", "Bildirim", 260)):
    ...
    def decompose_main_goal(self: Any) -> None:
        ...
        if not goal:
            ...
            return
        if not messagebox.askyesno("ZeroScript", "Görev analiz, geliştirme ve test adımlarına ayrılıp bağımlı kuyruğa eklensin mi?"):
            return
    ...
    def restore_instances(self: Any) -> None:
        checkpoint = ((self.last_status.get("checkpoint") or {}).get("latest") or
                      ((self.last_status.get("automation") or {}).get("instanceBackup") or {}).get("latest"))
        if not checkpoint:
            messagebox.showwarning("ZeroScript", "Geri alınabilecek checkpoint bulunamadı.")
            return
        if messagebox.askyesno("ZeroScript", f"{checkpoint} checkpoint'indeki script, UI, map ve Instance kapsamı geri yüklensin mi?"):
            self.action("restore_instances", {"id": checkpoint})

    def save_automation(self: Any) -> None:
        try:
            timeout = int(self.automation_timeout_var.get())
            recoveries = int(self.automation_recovery_var.get())
        except ValueError:
        ...
        payload = {
            "settings": {
                ...
                "maxTimeoutRecoveries": recoveries,
        ...
        self.action("set_automation", payload)
    ...
    def save_project_profile(self: Any) -> None:
        ...
        profiles[keys[0]] = {**dict(self.settings), "_projectKeys": keys, "_savedAt": int(time.time() * 1000)}
    ...
    def load_project_profile(self: Any, notify: bool = False) -> bool:
        ...
        if not matched:
            if notify:
                messagebox.showinfo("ZeroScript", "Bu oyun için kayıtlı profil yok. 'Bu oyuna profil kaydet' düğmesini kullan.")
            return False
        self.settings = {**hub.DEFAULT_SETTINGS, **profiles[matched]}
    ...
    def refresh_automation(self: Any) -> None:
        ...
        if project:
            self.automation_project_label.configure(text=f"Açık oyun: {project.get('name') or '-'} · PlaceId {project.get('placeId') or 0}")
            auto_key = str(project.get("key") or project.get("placeId") or "")
            if settings.get("autoProfile", True) and auto_key and getattr(self, "_zs_seen_project_key", None) != auto_key:
        ...
        for entry in automation.get("providerTable") or []:
            ...
            self.automation_models.insert("", END, text=str(entry.get("provider", "")), values=(f"%{entry.get('successRate', 0)}", entry.get("attempts", 0), avg, errors))
            row = self.automation_models.get_children()[-1]
            self.automation_models.item(row, tags=(str(entry.get("provider", "")),))
            self.automation_models.heading("#0", text="Model")
            self.automation_models.column("#0", width=90, stretch=True)

        groups = automation.get("errorGroups") or []
        error_lines = []
        for entry in groups[:15]:
            error_lines.append(f"{entry.get('count', 1)}x · {entry.get('line', '')}")
        self.automation_errors.delete("1.0", END)
        self.automation_errors.insert("1.0", "\n\n".join(error_lines) if error_lines else "Gruplanmış Output hatası yok.")

        diagnosis = automation.get("diagnosis") or {}
        rows = diagnosis.get("rows") or []
        lines = [f"Durum: {diagnosis.get('status', 'idle')}", str(diagnosis.get("detail", "")), ""]
        lines.extend(f"{'✓' if row.get('ok') else '✗'} {row.get('label', '')}: {row.get('detail', '')}" for row in rows)
        ...
        for notice in reversed((automation.get("notifications") or [])[-30:]):
            stamp = time.strftime("%H:%M", time.localtime(float(notice.get("at", 0) or 0) / 1000))
            self.automation_notifications.insert("", END, values=(stamp, notice.get("level", "info"), notice.get("title", "")))
